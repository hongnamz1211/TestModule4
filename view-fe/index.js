...
function getCity(city) {
    return `<tr>
                ...
                <td>      
                    <a href="#" onclick="showEdit(${city.id},2)">Chỉnh sửa</a> |
                     <a href="#" onclick="showDelete(${city.id})">Xóa</a>
                </td>
            </tr>`;
}
...
function showDelete(cityId) {
    ...
    document.getElementById("btn_home").innerHTML = `<button onclick="closeDetailForm()" class="btn btn-secondary">Thoát</button>`
...
function showEdit(cityId,com) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#national1').val(data.national.id);
            $('#idcity1').val(data.id);
            $.ajax({
                type: "GET",
...
function updateCity(){
    ...
    if (name === '' || description === '' || gdp < 0 || population < 0 || area < 0 || gdp === '' || population === '' || area === '') {
